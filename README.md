...
### React
```
React는 SPA (Single Page Application) 방식으로, 여러개의 페이지를 사용하며, 새로운 페이지를 로드하는 기존의 MPA 방식과 달리 
새로운 페이지를 로드하지 않고 하나의 페이지 안에서 필요한 데이터만 가져오는 형태
```

### MPA 방식
```
로그인.html 페이지와 회원가입.html가 별도로 나눠져 있고,
...
사용자가 회원가입 버튼을 눌렀을 때, 회원가입에 대한 데이터 자료를 렌더링하는 방식
```
...
MPA 방식에서 다른 페이지로 이동하기 위해선 주로 <a href=''> 태그를 사용하거나 window.location.href 등의 메소드를 사용할 수 있음
페이지와 페이지 사이 (html → html)의 이동이 가능하기 때문

SPA 방식에서는 오직 하나의 페이지만 가지고 있기 때문에 MPA 방식으로의 페이지 이동이 불가능함
페이지 (html)가 아닌 데이터를 가지는 JS 파일로 구성이 되어 있기 때문
